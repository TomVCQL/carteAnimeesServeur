    ...
    <div class="row">
        ...
        <div class="col-md-10 content">
            <div class="card m-3 text-center">
                <h1 style="color:#0d6efd">Demandes de suppression</h1>
                <div class="card-body">
                    <?php
                    if($_SESSION['role']=='admin'){
                        $stmt = $db->prepare("SELECT * FROM demandesuppression");
                        $stmt->execute();
                        $row = $stmt->fetchAll(PDO::FETCH_ASSOC);

                    }else{
                        $stmt = $db->prepare("SELECT * FROM demandesuppression WHERE Utilisateur = :id");
                    ...
                    <table id="myTable" class="table table-striped table-sm text-center display">
                        ...
                        <tbody>
                            ...
                            foreach ($row as $element) {
                                ...
                                }elseif ($element['TypeObjet'] == 'carte') {

                                    $stmt = $db->prepare("SELECT * FROM carte WHERE Id = :id");
                                    $stmt->execute(['id' => $element['ValeurIdentifiant']]);
                                    $carte = $stmt->fetch(PDO::FETCH_ASSOC);

                                    echo "<td>" . $carte['Intitule'] . "</td>";
                                    echo "<td></td>";
                                }elseif ($element['TypeObjet'] == 'serie'){

                                    $stmt = $db->prepare("SELECT * FROM serie WHERE Id = :id");
                                    $stmt->execute(['id' => $element['ValeurIdentifiant']]);
                                ...
                                if($_SESSION['role']=='admin'){
                                    ...
                                    <form class='form-horizontal' action='../controller/demande.php' method='post' enctype='multipart/form-data' style='float: left; margin-right: 10px;' onsubmit='return confirmSubmit(\"" . $element['TypeObjet'] . "\")'>
                                        
                                        <input type='hidden' name='typeDemande' value='valider'/>
                                        <input type='hidden' name='fonction' value='suppression' />
...
<script>
    ...
    function confirmSubmit(intitule) {
        return confirm("Êtes-vous sûr de vouloir supprimer l'element de type " + intitule + " ?");
    }
</script>